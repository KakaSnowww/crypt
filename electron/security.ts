...
export function isTrustedApplicationUrl(value: string, developmentServerUrl?: string) {
  try {
    ...
    if (!developmentServerUrl || candidate.username || candidate.password) {
      return false;
    }
...
export function isAllowedCryptDeepLink(value: string) {
  try {
    ...
    if (candidate.hostname === 'connections') {
      if (candidate.pathname !== '/callback') {
        return false;
      }
    ...
    if (candidate.hostname === 'arcana') {
      return (
        candidate.pathname === '/callback' && candidate.searchParams.get('status') === 'return'
      );
    }
