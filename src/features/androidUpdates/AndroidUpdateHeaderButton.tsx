...
import { releaseNoteLines } from '../desktopUpdates/releaseNotes';
import { useAndroidUpdates } from './useAndroidUpdates';

export function AndroidUpdateHeaderButton() {
  const { download, install, state } = useAndroidUpdates();
  const [open, setOpen] = useState(false);

  if (!isAndroidRuntime() || !['available', 'downloading', 'ready'].includes(state.state)) {
    return null;
  }
  ...
  const notes = releaseNoteLines(state.release?.notes);
  const label = ready
    ? `Instalar atualização ${state.release?.version ?? ''}`.trim()
    : downloading
      ? `Baixando atualização: ${state.percent ?? 0}%`
      : `Atualização ${state.release?.version ?? ''} disponível`.trim();
  ...
  return (
    <>
      ...
      <Modal
        description={`Versão instalada: ${state.currentVersion} · Nova versão: ${state.release?.version ?? 'disponível'}`}
        footer={
          ready ? (
        ...
        open={open}
        title={state.release?.name ?? 'Atualização do Crypt'}
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-xs leading-5 text-crypt-muted">
                {ready
                  ? 'O Android pedirá sua confirmação antes de substituir a versão atual.'
                  : 'O download usa o arquivo oficial publicado no GitHub Releases.'}
              </p>
            </div>
          </div>
          {downloading ? (
        ...
        {state.message ? (
          <p className="mt-4 rounded-xl border border-amber-400/15 bg-amber-500/[0.06] p-3 text-xs leading-5 text-amber-100">
            {state.message}
          </p>
        ) : null}
